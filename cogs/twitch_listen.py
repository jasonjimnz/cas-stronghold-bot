...
class TwitchListener(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        with open(self.config_path) as json_file: # add path thing
            ...
            self.channel = config_data['twitch_text_channel_id']
    ...
    @app_commands.command(name = "config-twitch", description = "Binds the twitch listener to a particular channel")
    @app_commands.checks.has_any_role('Staff', 'Admin')
    async def set_twitch_announce(self, interaction: Interaction, channel : TextChannel):
        ...
        config_data['twitch_text_channel_id'] = self.channel.id
...
def is_TwitchOnline(client, secret, user):
    ...
    ---------------
    client: String
        the ID for the twitch client
    secret: String
        the string for the Twitch secret
    user: String
        the twitch username, such as 'Kiver' to listen to
        the channel 'https://twitch.tv/Kiver
    ---------------
    Returns boolean on whether Twitch channel is online.    
    '''
    try:
